            ...
            >
                <div class="bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="mb-3 flex items-center gap-3">
                        ...
                        <h3 id="dialog-title" class="text-base font-semibold text-white">{{ $addbarang }}</h3>
                    </div>

                    @if ($errors->any())
                        <div wire:key="create-item-error-banner"
                    ...
                    <form class="mt-4 space-y-4" wire:submit.prevent="store" novalidate>
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div>
                                <label for="namaBarang" class="block text-sm text-gray-300">Nama Barang</label>
                                <input id="namaBarang" type="text" autocomplete="off"
                                ...
                                @error('namaBarang')
                                    <p wire:key="error-namaBarang" class="mt-1 text-xs text-red-400">{{ $message }}</p>
                                @enderror
                            </div>

                            <div>
                                <label for="merk" class="block text-sm text-gray-300">Merk</label>
                                <input id="merk" type="text" autocomplete="off"
                                ...
                                @error('merk')
                                    <p wire:key="error-merk" class="mt-1 text-xs text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div>
                                <label for="kodeBarangBmn" class="block text-sm text-gray-300">Kode Barang BMN</label>
                                <input id="kodeBarangBmn" type="text" autocomplete="off"
                                ...
                                @error('kodeBarangBmn')
                                    <p wire:key="error-kodeBarangBmn" class="mt-1 text-xs text-red-400">{{ $message }}</p>
                                @enderror
                            </div>

                            <div>
                                <label for="kategori" class="block text-sm text-gray-300">Kategori</label>
                                <input id="kategori" type="text" autocomplete="off"
                                ...
                                @error('kategori')
                                    <p wire:key="error-kategori" class="mt-1 text-xs text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                            <div class="sm:col-span-2">
                                <label for="lokasi" class="block text-sm text-gray-300">Lokasi</label>
                                <input id="lokasi" type="text" autocomplete="off"
                                ...
                                @error('lokasi')
                                    <p wire:key="error-lokasi" class="mt-1 text-xs text-red-400">{{ $message }}</p>
                                @enderror
                            </div>

                            <div>
                                <label for="kondisi" class="block text-sm text-gray-300">Kondisi</label>
                                <select id="kondisi" wire:model.live="kondisi"
                                ...
                                @error('kondisi')
                                    <p wire:key="error-kondisi" class="mt-1 text-xs text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div>
                                <label for="jumlah" class="block text-sm text-gray-300">Jumlah</label>
                                <input id="jumlah" type="number" min="1" inputmode="numeric"
                                    ...
                                    placeholder="0">
                                @error('jumlah')
                                    <p wire:key="error-jumlah" class="mt-1 text-xs text-red-400">{{ $message }}</p>
                                @enderror
                            </div>

                            <div>
                                <label for="tahunPengadaan" class="block text-sm text-gray-300">Tahun Pengadaan</label>
                                <input id="tahunPengadaan" type="number" inputmode="numeric"
                                ...
                                @error('tahunPengadaan')
                                    <p wire:key="error-tahunPengadaan" class="mt-1 text-xs text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                        </div>

                        <div>
                            <label for="superadmin-photo" class="block text-sm text-gray-300">Foto Barang (Opsional)</label>
                            <input id="superadmin-photo" type="file" wire:model="photo" accept="image/*"
                            ...
                            @error('photo')
                                <p wire:key="superadmin-error-photo" class="mt-1 text-xs text-red-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            <label for="keterangan" class="block text-sm text-gray-300">Keterangan (Opsional)</label>
                            <textarea id="keterangan" rows="3" wire:model.live.debounce.400ms="keterangan"
                            ...
                            @error('keterangan')
                                <p wire:key="error-keterangan" class="mt-1 text-xs text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
